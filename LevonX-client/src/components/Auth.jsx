...
const steps = [
  { 
    ...
    desc: "Quick and secure authentication to access all AI-powered features instantly." 
  },
  { 
    icon: HiSparkles, 
    title: "Unlock Free Credits",      
    desc: "Receive 150 AI credits to start building and experimenting without limits." 
  },
  { 
    icon: TbSettings, 
    title: "Customize Your Design",     
    desc: "Adjust props, styles, and layouts with real-time preview and control." 
  },
  { 
    icon: TbCopy,     
    title: "Generate UI Components", 
    desc: "Let AI create clean, scalable, and production-ready React components." 
  },
  { 
    icon: TbDownload, 
    title: "Export Your Code",        
    desc: "Copy or download optimized code and integrate it directly into your project." 
  },
];
function Auth({onClose}) {
  ...
  return (
    ...
    className='fixed inset-0 flex items-center justify-center bg-black/70 background-blur-sm z-50 p-4'>
        <motion.div 
    ...
    transition={{duration:0.5}}
        ...
        {/* Cros button */}
        <button onClick={onClose} className='absolute top-3 right-3 z-20 w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 flex items-center cursor-pointer'><TbX size={15}/></button>
        {/* Left BOX */}
    ...
    transition={{delay:0.2}}
            ...
            <div className='flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible pb-2 sm:pb-0 -mx-1 px-1'>
                {
                    steps.map((item,i)=>(
                        <motion.div key={i} className={`flex-shrink-0 sm:flex-shrink flex items-start gap-3 px-3 py-2.5 rounded-xl border transition-all duration-300 min-w-[200px] sm:min-w-0 ${active===i? "bg-[#3be8ff]/[0.07] border-[#3be8ff]/20":"bg-transparent border-transparent"}`}>
                            ...
                            <div>
                                <p className={`text-[12.5px] font-semibold transition-colors duration-300 whitespace-nowrap sm:whitespace-normal ${active === i ? "text-[#d4f5fa]" : "text-white/55"}`}>
                                    {item.title}
                                </p>
        ...
        <div className='relative z-10 w-full max-w-[280px] sm:mx-w-[260px] text-center mx-auto'>
            <motion.div
            animate={{y:[0,-6,0]}}
            ...
            <SiValorant size={17} color="#3be8ff"/>

            </motion.div>
            <h3 className='text-xl font-bold text-[#e4f6f8] tracking-tight mb-2' style={{fontFamily:"'Syne,sans-serif'"}}>
                Welcome to levonX UI
            </h3>
            <p className='text-[13px] text-[#96bec8]/55 leading-relaxed mb-6 sm:mb-7'>
                Experience the future of UI development with AI-powered design and code generation. Sign in to unlock your creativity and build stunning interfaces effortlessly.
            </p>
            <div className='flex justify-center gap-4 sm:gap-5 mb-6 sm:mb-7'>
                {
                    [["150","AI Credits"],["24/7","Support"],["100%","Free"]].map(([v,l],i)=>(
            ...
            <p className='text-[11px] text-[#64919b]/45 mt-4 sm:mt-5'>
                ...
                <span onClick={onClose} className="text-[#3be8ff]/50 border-b border-[#3be8ff]/20 cursor-pointer hover:text-[#3be8ff]/8 transition-colors">
                    View docs
                </span>
